...
Architecture
------------
Compressor (1258 -> hidden) -> 2 residual blocks -> shared policy stem
...
class PlacementModel(nn.Module):
    ...
    def __init__(self, obs_size: int = 1258, hidden_size: int = 256):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor):
        ...
        x = nn.functional.relu(x + self.residual1(x))
        x = nn.functional.relu(x + self.residual2(x))
